...
def BM3D_2nd_step(_basicImg, _noisyImg):
    ...
    width, height = _noisyImg.shape[0], _noisyImg.shape[1]
...
if __name__ == '__main__':
    ...
    # if(img is not None):
    #     print("success")
    Basic_img_b = BM3D_1st_step(b)
    Basic_img_g = BM3D_1st_step(g)
    Basic_img_r = BM3D_1st_step(r)
    Basic_img=cv2.merge((Basic_img_b,Basic_img_g,Basic_img_r))

    e2 = cv2.getTickCount()
    time = (e2 - e1) / cv2.getTickFrequency()   # 计算函数执行时间
    print ("The Processing time of the First step is %f s" % time)
    cv2.imwrite("Basic1.jpg", Basic_img)
    ...
    Final_img, _ = BM3D_2nd_step(Basic_img, img)
    e3 = cv2.getTickCount()
